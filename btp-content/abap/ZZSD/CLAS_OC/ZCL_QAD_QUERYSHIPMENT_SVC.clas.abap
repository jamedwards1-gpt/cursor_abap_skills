CLASS zcl_qad_queryshipment_svc DEFINITION
  ...
  PUBLIC SECTION.
    ...
    CONSTANTS:
      BEGIN OF gc_poll_status,
        submitted TYPE zqad_poll_log-status VALUE 'SUBMITTED',
        polling   TYPE zqad_poll_log-status VALUE 'POLLING',
        complete  TYPE zqad_poll_log-status VALUE 'COMPLETE',
        error     TYPE zqad_poll_log-status VALUE 'ERROR',
  ...
  PRIVATE SECTION.
    ...
    METHODS update_poll_log
      IMPORTING
        iv_log_uuid        TYPE zqad_poll_log-shipment_uuid
        iv_status          TYPE zqad_poll_log-status
        iv_status_message  TYPE string
...
CLASS zcl_qad_queryshipment_svc IMPLEMENTATION.
  ...
  METHOD apply_poll_log_outcome.
    ...
    DATA lv_status TYPE zqad_poll_log-status.
  ...
  METHOD execute_shipment_query.
    ...
    IF iv_use_mock_data = abap_false AND lv_response_xml IS INITIAL.
      RAISE EXCEPTION TYPE zcx_qad_simple_error EXPORTING message = 'QAD API call returned an empty response.'.
    ENDIF.
  ...
  METHOD poll_pending_queue.
    DATA lt_queue TYPE STANDARD TABLE OF zqad_poll_log.
    DATA ls_ship_key TYPE ty_qad_shipment_key.
    DATA lt_tracking TYPE tt_qad_tracking_info.
    DATA lt_errors TYPE tt_error_messages.
    DATA lv_kind TYPE ty_response_kind.
  ...
  METHOD query_shipment_info.
    ...
    TRY.
      ...
      CATCH zcx_qad_simple_error INTO DATA(lx_query_error).
        apply_poll_log_outcome(
  ...
  METHOD resolve_next_sequence.
    ...
    DATA lv_poll_ref TYPE zqad_poll_log-shipment_reference.
